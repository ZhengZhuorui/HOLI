...
template<typename _Key, typename _Val, typename traits>
inline bool aex_tree<_Key, _Val, traits>::insert_merge(inner_node_ptr parent, const key_type &new_key, node_ptr new_node){
    // check nodes can merge
    // AllowInsertBalance need open
    AEX_ASSERT(parent != nullptr);
    if constexpr (traits::AllowInsertBalance)
        return false;
    //if (IS_LEAF_NODE(new_node))
    //    return false;
    AEX_ASSERT(IS_LEAF_NODE(new_node) == false);
    // child must not inner node
    //if (!IS_ML_NODE(parent))
    //    return false;
    AEX_ASSERT(IS_ML_NODE(parent) == false);
    //if (!CAN_MERGED_NODE(new_node))
    //    return false;
    AEX_ASSERT(CAN_MERGED_NODE(new_node) == false);
    AEX_HINT("INSERT_MERGE");
    
    slot_type pos = parent->find(new_key), pred_pos = parent->predict(new_key);
    ...
    inner_node_ptr node_merge_buf[traits::ERROR_BOUND << 1];
    auto insert_merge_node = [&](key_type key, node_ptr node){key_merge_buf[node_merge_size] = key; node_merge_buf[node_merge_size] = static_cast<inner_node_ptr>(node); node_merge_size++;};
    for (slot_type i = pred_pos; i < pos; ++i)
    if (bitmap_impl::at(parent->bitmap_ptr, i) != 0){
        if  (!CAN_MERGED_NODE(parent->child_ptr[pos]))
            return false;
        else{
...

    {
        ...
        //   2.3 + parent delta SMO probability in future
        // 3. -parent delta write cost this times
        ...
        std::for_each(node_merge_buf, node_merge_buf + node_merge_size, [&](inner_node_ptr node){
        ...
        double parent_write_cost = 1.0 * SMO_times / lambda_timestamp * traits::MODEL_ARGS::GAP_ARRAY_INSERT_FACTOR;// + 
        double merge_parent_cost = -1.0 * (1.0 / lambda_timestamp) * parent->size * traits::MODEL_ARGS::INNER_NODE_TRAIN_FACTOR;// -   
        double delta_cost = read_cost + SMO_cost + parent_write_cost + merge_parent_cost;
        AEX_PRINT("delta cost: " << delta_cost);
        if (delta_cost >= 0) return false;
    }
    
    inner_node_ptr merge_node;
    ...
    for (slot_type i = 0, j = 0; i < node_merge_size; j += node_merge_buf[i]->size, ++i)
        copy_to_buffer(node_merge_buf[i], key_buf + j, child_buf + j);
    ...
    if (ret){
        ...
        std::for_each(node_merge_buf, node_merge_buf + node_merge_size, [&](node_ptr node){allocator.free_node(node);});
    }
    return ret;
}
...
//        allocator.reallocate(new_node, min_slot_size(data_size, traits::MIN_DATA_NODE_SLOT_SIZE));
//
...
template<typename _Key, typename _Val, typename traits>
inline int aex_tree<_Key, _Val, traits>::check_split(inner_node_ptr node){
    // delta cost
...
inline typename aex_tree<_Key, _Val, traits>::slot_type aex_tree<_Key, _Val, traits>::check_split_size(inner_node_ptr node){
    ...
    do{
        ...
        delta_cost = read_cost + write_cost + SMO_cost;
        //AEX_PRINT("delta cost=" << delta_cost << ", read_cost=" << read_cost << ", write_cost=" << write_cost << ", SMO_cost=" << SMO_cost);
