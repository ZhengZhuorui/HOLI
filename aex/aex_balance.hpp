...
template<typename _Key, typename _Val, typename traits>
inline bool aex_tree<_Key, _Val, traits>::check_insert_merge(node_ptr* node_buffer, slot_type size){
    // delta cost
    // 1. write cost 
    // 2. read cost
    // 3. merge cost
...
template<typename _Key, typename _Val, typename traits>
inline void aex_tree<_Key, _Val, traits>::merge_nodes(key_type* key_buffer, data_node_ptr* node_buffer, slot_type size){
...
template<typename _Key, typename _Val, typename traits>
inline void aex_tree<_Key, _Val, traits>::merge_nodes(key_type* key_buffer, inner_node_ptr* node_buffer, slot_type size){
    #ifdef AEX_EXPERIMENT
...
inline bool aex_tree<_Key, _Val, traits>::check_split(inner_node_ptr node){
    ...
    if (delta_cost < 0){
        ...
        AEX_PRINT("node=" << node << ", train_times=" << node->balance_stats.get_train_times() << ", write_times=" << node->balance_stats.get_write_times());
...
inline typename aex_tree<_Key, _Val, traits>::slot_type aex_tree<_Key, _Val, traits>::check_split_size(inner_node_ptr node){
    ...
    double train_pro = 1.0 * node->balance_stats.get_train_times() / lambda_timestamp;
    double write_pro = 1.0 * node->balance_stats.get_write_times() / lambda_timestamp;
    double read_cost, SMO_cost, write_cost, delta_cost;
    ...
    do{
        ...
        SMO_cost = -train_pro * traits::MODEL_ARGS::INNER_NODE_TRAIN_FACTOR * node->size + train_pro * traits::MODEL_ARGS::INNER_NODE_TRAIN_FACTOR * (node->size / split_nums);
        write_cost = (IS_ML_NODE(node)) ? 0 : -1.0 * write_pro * (node->size / split_nums) * traits::MODEL_ARGS::DENSE_ARRAY_INSERT_FACTOR;
        delta_cost = read_cost + SMO_cost + write_cost;
