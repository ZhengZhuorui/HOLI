...
void aex_tree<_Key, _Val, traits>::_get_info_stats(const node_ptr node, const unsigned int depth, info_stats& stats) const {
    ...
    switch (node->type){
        ...
        case NodeType::HashNode:{
            ...
            stats.hash_table_memory_used += h_n(node)->hash_table.memory_used();
            int cnt = 0;
...
info_stats aex_tree<_Key, _Val, traits>::get_info_stats() const {
    ...
    if (this->root->type == NodeType::HashNode)
    AEX_WARNING("root->slot_size=" << h_n(root)->slot_size << ", hash_table.slot_size=" << h_n(root)->hash_table.slot_size);
