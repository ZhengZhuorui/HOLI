...
inline void aex_tree<_Key, _Val, traits>::update(hash_node_ptr parent, const slot_type pos, const slot_type next_pos, const node_ptr old_node, const key_type new_key, const node_ptr new_node){
    ...
    parent->hash_table.update(pos, std::make_pair(new_key, new_node));
    for (slot_type i = highbit<slot_type, traits::SLOT_PER_SHORTCUT>(pos + 1); i < next_pos; i += traits::SLOT_PER_SHORTCUT)
        parent->hash_table.update(i, std::make_pair(new_key, new_node));
}
...
inline bool aex_tree<_Key, _Val, traits>::expand(dense_node_ptr node){
    ...
    AEX_ASSERT(node->is_train == false);
    AEX_ASSERT(node->level > 1);
    #ifdef AEX_DEBUG
    ...
    if (slot_size == 0)
        ...
        AEX_PRINT("[dense node expand]slot_size=" << slot_size);
        clear_childs_recursive(node);
        cast_to_hash_node(node, slot_size);
        reinterpret_cast<hash_node_ptr>(node)->model = m;
        construct_hash_node(reinterpret_cast<hash_node_ptr>(node), key_buf.data(), child_buf.data(), child_buf.size());
...
inline void aex_tree<_Key, _Val, traits>::expand(hash_node_ptr node){
    ...
    construct_hash_node(node, key_buf.data(), child_buf.data(), child_size);
    #ifdef AEX_DEBUG
...
inline typename aex_tree<_Key, _Val, traits>::slot_type aex_tree<_Key, _Val, traits>::train(const key_type* const keys, const ULL n, Model &m, const double few_ratio) {
    ...
    if (ans > 0){
        ...
        AEX_PRINT("occupied=" << occupied << ", slot_size=" << ans);
    }
    return ans;
}
