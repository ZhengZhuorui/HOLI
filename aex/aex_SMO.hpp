...
bool aex_tree<_Key, _Val, traits>::update_childnode_key(inner_node_ptr __restrict__ parent, const node_ptr __restrict node, const key_type &key){
    ...
    if (parent->prop & node_property::ML_NODE){
        ...
        if (old_pos == new_pos){
            size_type next_pos = parent->next_item(old_pos);
            for (size_type i = new_pos; i < next_pos; ++i)
                node_key[i] = key;
            return true;
        }
            ...
            if (bitmap_impl::at(bm, new_pos) == 0){
                if (old_pos > 0)
                    for (size_type i = old_pos; i < new_pos; ++i){
                        node_key[i] = node_key[old_pos - 1];
                        child[i] = child[old_pos - 1];
                    }

                size_type next_pos = parent->prev_item(new_pos);
                for (size_type i = new_pos; i < next_pos; ++i){
                    node_key[i] = key;
                }
            ...
            if (bitmap_impl::at(bm, new_pos) == 0){
                for (size_type i = new_pos; i < old_pos; ++i)
                    node_key[i] = key;
                bitmap_impl::set_zero(bm, old_pos);
                bitmap_impl::set_one(bm, new_pos);
                return true;
...
bool aex_tree<_Key, _Val, traits>::update_childnode_ptr(inner_node_ptr __restrict__ parent, const node_ptr old_node, const node_ptr new_node){
    ...
    if (parent->prop & node_property::ML_NODE){
        size_type next_pos = parent->next_item(pos);
...
void aex_tree<_Key, _Val, traits>::split(data_node_ptr __restrict__ old_node, data_node_ptr __restrict__ new_node){
    ...
    memcpy(new_node->key, old_node->key + (old_node->size - mid), (old_node->size - mid) * sizeof(key_type));
    memcpy(new_node->data, old_node->data + (old_node->size - mid), (old_node->size - mid) * sizeof(value_type));
...
bool aex_tree<_Key, _Val, traits>::split_with_old_node(const key_type* const __restrict__ key, const node_ptr* const __restrict__ child, const size_type n, 
    ...
    AEX_FORMAT("target 0, node->slot_size=%lld", node->slot_size);
...
void aex_tree<_Key, _Val, traits>::split(const key_type* const __restrict__ key, const node_ptr* const __restrict__ child, const unsigned int n, const unsigned int level, std::vector<key_type> &new_key, std::vector<node_ptr> &new_child){
    ...
    while (start < end){
        ...
        for (; slot_size < (end - start) * this->inner_node_few_ratio[level] && slot_size; slot_size <<= 1){
...
void aex_tree<_Key, _Val, traits>::split(const key_type* const __restrict__ key, const value_type* const __restrict__ data, const unsigned int n, std::vector<key_type> &new_key, std::vector<node_ptr> &new_child){
    ...
    while (start < end){
        ...
        for (; slot_size < end - start && slot_size < traits::MAX_DATA_NODE_SLOT_SIZE; slot_size <<= 1){
        ...
        slot_size = std::max(traits::MIN_DATA_NODE_SLOT_SIZE, slot_size);
        size_type size = std::min(slot_size, end - start);
...
template<typename _Key, typename _Val, typename traits>
typename traits::size_type aex_tree<_Key, _Val, traits>::linear_probe(const key_type* const key, const size_type n, data_node_model &m){
    ...
    std::pair<size_type*, size_type*> sta = node_allocator.allocate_size_type_buffer(n);
    size_type* sta1 = sta.first;
    size_type* sta2 = sta.second;
    ...
    for (size_type i = 2; i < n; ++i){
        double ERROR = traits::MAX_LINEAR_PROBE_ALLOW_ERROR * log(std::max(static_cast<size_type>(traits::MIN_ML_DATA_NODE_SLOT_SIZE), i));
        ...
        while (top1 > 1 && cross_product(key[sta1[top1 - 2]], sta1[top1 - 2], key[sta1[top1 - 1]], sta1[top1 - 1], key[i], i) <= 0) --top1;
        sta1[top1++] = i;

        while (top2 > 1 && cross_product(key[sta2[top2 - 2]], sta2[top2 - 2], key[sta2[top2 - 1]], sta2[top2 - 1], key[i], i) >= 0) --top2;
        sta2[top2++] = i;
    ...
    for (size_type i = 0; i < ret; ++i){
        sum += 1.0 * i / (ret - 1) - m.args.slope * (key[i] - key[0]);
    }
    m.args.inter = sum / ret;
    node_allocator.deallocate(sta);
    return ret;
}

// split a ordered key array with data array to inner node array. Using linear probe
template<typename _Key, typename _Val, typename traits>
void aex_tree<_Key, _Val, traits>::split_with_linear_probe(const key_type* const __restrict__ key, const value_type* const __restrict__ data, const unsigned int n, std::vector<key_type> &new_key, std::vector<node_ptr> &new_child){
    size_type start = 0, end = n;
    
    while (start < end){
        data_node_model model;
        size_type ret = linear_probe(key + start, end - start, model);
        size_type slot_size = traits::MIN_DATA_NODE_SLOT_SIZE, size;
        while (slot_size < ret) slot_size <<= 1;
...
template<typename _Key, typename _Val, typename traits>
bool aex_tree<_Key, _Val, traits>::check_rewired(const key_type* const __restrict__ key, const size_type size, const size_type slot_size, inner_node_model &m){
    if (slot_size < traits::MIN_ML_INNER_NODE_SLOT_SIZE)
        return true;
    size_type pos;
    m.train(key, size);
    //AEX_PRINT("RMSE=" << m.RMSE(key, size) << ", slot_size=" << slot_size);
    //AEX_FORMAT("BEGIN");
    for (size_type i = 0, start=0; i < size; ++i){            
        pos = std::max((size_type)0, std::min((size_type)(round(m.predict(key[i]) * slot_size)), slot_size - 1));
        //AEX_PRINT("key=" << key[i] << ", pos=" << pos << ", pred=" << m.predict(key[i]));
        ...
        if (start - pos >= traits::ERROR_BOUND || start > slot_size + traits::ERROR_BOUND){
            //AEX_FORMAT("start=%lld pos=%lld", start, pos);
            //AEX_FORMAT("RETURN FALSE");
            return false;
        }
        ++start;
    }
...
bool aex_tree<_Key, _Val, traits>::rescale(inner_node_ptr __restrict__ &node, inner_node_ptr __restrict__ parent, const double ratio){
    AEX_FORMAT("BEGIN");
    AEX_FORMAT("real_slot_size=%lld, EXPAND_RATIO=%.2f, node level=%d", node->real_slot_size(), traits::EXPAND_RATIO, node->level);
...
void aex_tree<_Key, _Val, traits>::copy_node(inner_node_ptr __restrict__ node, inner_node_ptr __restrict__ new_node){
    ...
    if (!(node->prop & node_property::ML_NODE) && !(new_node->prop & node_property::ML_NODE)){
        //AEX_DEBUG_FORMAT("copy node 1 " << node << " "<< new_node);
...
void aex_tree<_Key, _Val, traits>::merge_to_left_leaf(data_node_ptr __restrict__ left_node, data_node_ptr __restrict__ right_node){
    ...
    memcpy(left_node->key + left_node->size, right_node->key, right_node->size * sizeof(key_type));
    memcpy(left_node->data + left_node->size, right_node->data, right_node->size * sizeof(value_type));
...
void aex_tree<_Key, _Val, traits>::merge_to_right_leaf(data_node_ptr __restrict__ left_node, data_node_ptr __restrict__ right_node){
    ...
    AEX_ASSERT((right_node->prop & node_property::ML_NODE) == 0);
    AEX_ASSERT(left_node != right_node);
    memmove(right_node->key + left_node->size, right_node->key, right_node->size * sizeof(key_type));
    memmove(right_node->data + left_node->size, right_node->data, right_node->size * sizeof(value_type));
    memcpy(right_node->key, left_node->key, left_node->size * sizeof(key_type));
    memcpy(right_node->data, left_node->data, left_node->size * sizeof(value_type));
...
void aex_tree<_Key, _Val, traits>::merge_to_right_node(inner_node_ptr __restrict__ left_node, inner_node_ptr __restrict__ right_node){
    ...
    memcpy(right_node->key_ptr, left_node->key_ptr, left_node->size * sizeof(key_type));
    memcpy(right_node->child_ptr, left_node->child_ptr, left_node->size * sizeof(node_ptr));
...
void aex_tree<_Key, _Val, traits>::shift_to_left_leaf(data_node_ptr __restrict__ left_node, data_node_ptr __restrict__ right_node){
    ...
    memmove(right_node->data, right_node->data + 1, (right_node->size - 1) * sizeof(value_type));
    ++left_node->size;
    --right_node->size;
}
...
void aex_tree<_Key, _Val, traits>::shift_to_right_leaf(data_node_ptr __restrict__ left_node, data_node_ptr __restrict__ right_node){
    ...
    memmove(right_node->data + 1, right_node->data, (right_node->size) * sizeof(value_type));
...
void aex_tree<_Key, _Val, traits>::shift_to_right_node(inner_node_ptr __restrict__ left_node, inner_node_ptr __restrict__ right_node){
    AEX_ASSERT((right_node->prop & node_property::ML_NODE) == 0);
    size_type shift_pos = left_node->last();
