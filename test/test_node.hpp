...
bool test_inner_node_insert_perf(vector<key_type> &data, long long n, long long batch){
    ...
    for (long long i = 0; i < n; ++i) node_data[i] = data[i];
    ...
    assert(static_cast<long long>(final_node_data.size()) == n + batch - insert_failed);
    ...
    for (int i = 0; i < node->slot_size; ++i){
        ...
        if (bit_cnt > 0 && i > 0 && bitmap_impl::at(node->bitmap_ptr, i)){
            if (node->key_ptr[i] == node->key_ptr[i - 1]){
                AEX_PRINT("Error slot[" << i << "] key changes, but bitmap is set");
                return false;
            }
            if (node->child_ptr[i] == node->child_ptr[i - 1]){
                AEX_PRINT("Error slot[" << i << "] child changes, but bitmap is set");
                return false;
            }
        }
        if (i > 0 && (!bitmap_impl::at(node->bitmap_ptr, i))){
            if (node->key_ptr[i] != node->key_ptr[i - 1]){
                AEX_PRINT("Error slot[" << i << "] key changes, but bitmap is empty");
                return false;
            }
            if (node->child_ptr[i] != node->child_ptr[i - 1]){
                AEX_PRINT("Error slot[" << i << "] child changes, but bitmap is empty");
                return false;
            }
        }
        if (bitmap_impl::at(node->bitmap_ptr, i)){
            if (node->key_ptr[i] != final_node_data[bit_cnt]){
                AEX_PRINT("Key error, node key[" << i<< "]=" << node->key_ptr[i] << ", real key=" << final_node_data[bit_cnt]);
                return false;
            }
        }
    ...
    if (bit_cnt != n + batch - insert_failed){
        AEX_PRINT("bit one cnt not equal items, 1 bits=" << bit_cnt << " n=" << n + batch - insert_failed);
        return false;
    }
    

    printf("test insert node performance");
...
template<typename key_type,
        typename value_type,
        typename traits=aex::aex_default_traits<key_type, value_type>>
bool test_inner_node_erase_perf(vector<key_type> &data, long long n, long long batch){
    ...
    vector<long long> del_pos(batch);
    ...
    generate_unique_dataset<long long, std::uniform_int_distribution<long long>, long long>(del_pos, batch, 1, n - 1);
    for (int i = 0; i < batch; ++i)
    ...
    for (int i = 0; i < node->slot_size; ++i){
        ...
        if (bitmap_impl::at(node->bitmap_ptr, i)){
            if (left_key[bit_cnt] != node->key_ptr[i]){
                AEX_PRINT("Key Error, node key[" << i << "]=" << node->key_ptr[i] << ", real key=" << left_key[bit_cnt]);
                return false;
            }
        }
        bit_cnt += ((bitmap_impl::at(node->bitmap_ptr, i)) != 0);
...
// test inner node find_lower_pos(key)
template<typename key_type,
        typename value_type,
        typename traits=aex::aex_default_traits<key_type, value_type> >
bool test_inner_node_query_perf(vector<key_type> &data, long long n, long long batch){
    ...
    generate_query(pack_data, query, answer, batch);
    for (int i = 0; i < batch; ++i){
        size_type pos = std::lower_bound(data.data(), data.data() + n, query[i]) - data.data();
        if (pos < static_cast<size_type>(n - 1))
    ...
    AEX_PRINT("construct finish.");
    for (int i = 0; i < batch; ++i){
        size_type pos = node->find_lower_pos(query[i]);
        if (node->child_ptr[pos] != answer[i]){
            AEX_PRINT("Query Error! query=" << query[i] << ", get pos=" << pos << ", get child=" << node->child_ptr[pos] << ", real child=" << answer[i]);
            return false;
        }
    }
...
template<typename key_type,
        typename value_type,
        typename traits=aex::aex_default_traits<key_type, value_type> >
bool test_inner_node_other(vector<key_type> &data, long long n, long long batch){
    return false;
}
...
template<typename key_type,
        typename value_type,
        typename traits=aex::aex_default_traits<key_type, value_type> >
bool test_inner_node(vector<key_type> &data, long long n){
    return false;
}
...
template<typename key_type,
        typename value_type,
        typename traits=aex::aex_default_traits<key_type, value_type> >
bool test_data_node(std::pair<key_type, value_type>* data, long long n){
    return false;
}
...
template<typename key_type,
        typename value_type,
        typename traits=aex::aex_default_traits<key_type, value_type> >
bool test_data_node_insert_perf(std::pair<key_type, value_type>* data, long long n, long long batch){
    AEX_PRINT("[test data node insertion performance]");
    mock_aex_tree<key_type, value_type, traits> tree;
...
template<typename key_type,
        typename value_type,
        typename traits=aex::aex_default_traits<key_type, value_type> >
bool test_data_node_query_perf(std::pair<key_type, value_type>* data, long long n, long long batch){
    ...
    if (!(node->prop & aex::node_property::ML_NODE)) return false;
    for (int i = 0; i < batch; ++i){
        size_type pos = std::lower_bound(ori_data.data(), ori_data.data() + n, query[i]) - ori_data.data();
        if (pos > 0)
            query[i] -= (1.0 * (rand() % 65536) / 65536) * (ori_data[pos] - ori_data[pos - 1]);
    }

    for (int i = 0; i < batch; ++i){       
        [[maybe_unused]] typename traits::size_type pos = node->find_lower_pos(query[i]);
        if (answer[i] != node->data[pos]){
            AEX_PRINT("answwer wrong! key=" << query[i] << ", answer=" << answer[i] << ", data=" << node->data[i]);
        }
    }
...
template<typename key_type,
        typename value_type,
        typename traits=aex::aex_default_traits<key_type, value_type> >
bool test_data_node_erase_perf(std::pair<key_type, value_type>* data, long long n, long long batch){
